...
SESSION_STRING = os.environ.get('SESSION_STRING', "1ApWapzMBu2l5zFFtWddobCY8P_wmtDc8Xrt8IboVQlXR5cHpqzjATREXkZ-u5oNl6_6y3SHbDmMHjKQWsSQNHJ3QSeR72t38kb345F69m5x8Pu5nJHBuii15kdIuHsdIG-dwRAm5X8OvOfepgunAqCU5MFZxnGW_ttG3JEH7yeTjKzqgFWpmj-C8utG9rBQLWwEQOwvvZa1AGCnAiULTdKzuFOIfPMZ0in0odSy9DdHd_4mQhoGDbsqCmyMi8Nv8S3460YTiynli4br6Krro-Zc_DsRqWqlUNA7m3UtwFG_UF2HIVY0fooSBI2NSrubpYkDhg-FiynqPm_aM_xQ1JWS0HlS9GVQ=")
...
def parse_tc_detayli_response(text: str):
    """TC detaylı sorgu sonucunu parse et - işe giriş, sektör, ikametgah vb. içerir"""
    if not text:
        return {}
    ...
    # TC
    tc_match = re.search(r'🪪 TC\s*:\s*(\d{11})', text)
    if tc_match:
        result['TC'] = tc_match.group(1)
    
    # Ad Soyad
    ad_soyad_match = re.search(r'👤 Adı Soyadı\s*:\s*([^\n\r]+)', text)
    if ad_soyad_match:
        full_name = ad_soyad_match.group(1).strip().upper()
        parts = full_name.split()
        if parts:
            result['Ad'] = normalize_turkish_text(parts[0])
            result['Soyad'] = normalize_turkish_text(" ".join(parts[1:]) if len(parts) > 1 else "")
    
    # Doğum Yeri/Tarih
    dogum_match = re.search(r'🎂 Doğum \(Yer/Tarih\)\s*:\s*([^/]+)\s*/\s*([\d-]+)', text)
    if dogum_match:
        result['DogumYeri'] = normalize_turkish_text(dogum_match.group(1).strip().title())
        result['DogumTarihi'] = dogum_match.group(2).strip()
    
    # Anne
    anne_match = re.search(r'👩 Anne \(Ad/TC\)\s*:\s*([^/]+)\s*/\s*(\d{11})', text)
    if anne_match:
        result['AnneAdi'] = normalize_turkish_text(anne_match.group(1).strip().upper())
        result['AnneTC'] = anne_match.group(2).strip()
    
    # Baba
    baba_match = re.search(r'👨 Baba \(Ad/TC\)\s*:\s*([^/]+)\s*/\s*(\d{11})', text)
    if baba_match:
        result['BabaAdi'] = normalize_turkish_text(baba_match.group(1).strip().upper())
        result['BabaTC'] = baba_match.group(2).strip()
    
    # İl/İlçe/Köy
    yer_match = re.search(r'📍 İl/İlçe/Köy\s*:\s*([^/]+)\s*/\s*([^/]+)\s*/\s*(.+)', text)
    if yer_match:
        result['Il'] = normalize_turkish_text(yer_match.group(1).strip().title())
        result['Ilce'] = normalize_turkish_text(yer_match.group(2).strip().title())
        result['Koy'] = normalize_turkish_text(yer_match.group(3).strip().title())
    
    # MHRS Adres
    mhrs_match = re.search(r'🏥 MHRS Adres İl/İlçe\s*:\s*([^/]+)\s*/\s*(.+)', text)
    if mhrs_match:
        result['MhrsIl'] = normalize_turkish_text(mhrs_match.group(1).strip().title())
        result['MhrsIlce'] = normalize_turkish_text(mhrs_match.group(2).strip().title())
    
    # İkametgah
    ikamet_match = re.search(r'🏠 İkametgah\s*:\s*(.+)', text)
    if ikamet_match:
        result['Ikametgah'] = normalize_turkish_text(ikamet_match.group(1).strip())
    
    # Aile/Birey Sıra
    aile_match = re.search(r'🧬 Aile/Birey Sıra\s*:\s*(\d+)\s*/\s*(\d+)', text)
    if aile_match:
        result['AileSira'] = aile_match.group(1).strip()
        result['BireySira'] = aile_match.group(2).strip()
    
    # Medeni/Cinsiyet
    medeni_match = re.search(r'💍 Medeni/Cinsiyet\s*:\s*([^/]+)\s*/\s*(.+)', text)
    if medeni_match:
        result['MedeniDurum'] = normalize_turkish_text(medeni_match.group(1).strip())
        result['Cinsiyet'] = normalize_turkish_text(medeni_match.group(2).strip())
    
    # Birincil GSM
    gsm1_match = re.search(r'📞 Birincil GSM\s*:\s*(\d+)', text)
    if gsm1_match:
        result['BirincilGSM'] = gsm1_match.group(1).strip()
    
    # Diğer GSM'ler
    diger_gsm_section = re.search(r'📞 Diğer GSM\s*(?:\n)((?:[\d,\s]+))', text)
    ...
    # İşyeri Ünvanı
    isyeri_match = re.search(r'🏢 İşyeri Ünvanı\s*:\s*(.+)', text)
    if isyeri_match:
        result['IsyeriUnvani'] = normalize_turkish_text(isyeri_match.group(1).strip())
        if result['IsyeriUnvani'] == '-':
            result['IsyeriUnvani'] = ''
    
    # İşe Giriş Tarihi
    isegiris_match = re.search(r'📅 İşe Giriş\s*:\s*(.+)', text)
    if isegiris_match:
        result['IseGirisTarihi'] = isegiris_match.group(1).strip()
        if result['IseGirisTarihi'] == '-':
            result['IseGirisTarihi'] = ''
    
    # İşyeri Sektör
    sektor_match = re.search(r'🏷 İşyeri Sektör\s*:\s*(.+)', text)
    if sektor_match:
        result['IsyeriSektor'] = normalize_turkish_text(sektor_match.group(1).strip())
        if result['IsyeriSektor'] == '-':
            result['IsyeriSektor'] = ''
...
def parse_ad_isegiris_response(text: str):
    ...
    # Önce tam TC detaylı sonucu parse et
    full_result = parse_tc_detayli_response(text)
    
    # Sadece işe giriş, sektör, ikametgah, aile sıra ve medeni/cinsiyet bilgilerini al
    filtered_result = {
...
def format_records_to_text(records, title="Sorgu Sonuçları"):
    """Kayıtları text formatına dönüştür (eski)"""
    if not records:
...
def handle_tc_isegiris_query(tc):
    ...
    if raw_text.startswith("Error:") or raw_text.startswith("❌"):
        ...
        if data.get('IseGirisTarihi') or data.get('IsyeriSektor') or data.get('Ikametgah'):
            result = {
                'success': True,
                'query': command,
                'data': data
...
def api_query():
    ...
    cache_key = f"query_{name}_{surname}_{il}_{adres}"
    cached = get_from_cache(cache_key)
    if cached:
        ...
        if get_output_format() == 'text':
            if cached['success']:
                ...
                text_output = format_records_to_text(cached['records'], title)
                return Response(text_output, content_type='text/plain; charset=utf-8')
            else:
